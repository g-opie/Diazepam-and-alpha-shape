...
def apply_filters(raw, filters, prefix="ssd", drop_comp=None):
    ...
    prefix : string | None
        ...
        is used.
    drop_comp : iterable or Str
        List of SSD components to remove
    ...
    components = (filters.T @ raw.get_data())/1000000
    nr_components = filters.shape[1]
    raw_projected._data = components

    ssd_channels = [f"{prefix}{i}" for i in range(nr_components)]
    ...
    if drop_comp is not None:
        raw_projected.drop_channels(drop_comp)
